...
fn is_clear_only_conversation(user_messages: &[String]) -> bool {
    ...
    // Check if all messages match the /clear pattern
    let has_caveat = user_messages.iter().any(|msg| {
        msg.starts_with("Caveat: The messages below were generated by the user while running local commands.")
    });

    let has_command_tags = user_messages.iter().any(|msg| {
        msg.contains("<command-name>/clear</command-name>")
    });

    let has_stdout_tags = user_messages.iter().any(|msg| {
        msg.contains("<local-command-stdout>")
    });

    // If we only have these specific messages, filter it out
